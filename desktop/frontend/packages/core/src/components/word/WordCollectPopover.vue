...
async function confirmCreateDict() {
  ...
  if (!result.ok) {
    if (result.reason === 'duplicate') Toast.warning('已有相同名称！')
    else if (result.reason === 'api') Toast.error('添加失败')
    return
  }
  cancelCreating()
  Toast.success('添加成功')
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div
        ...
        @click.stop
      >
        <div class="font-medium mb-2 pr-1">{{ '收藏到词典' }}</div>
        ...
        <div v-if="dictList.length" class="dict-list">
          ...
          >
            ...
            <span v-if="index < 9" class="dict-shortcut">{{ '快捷键' }} {{ index + 1 }}</span>
          </div>
        </div>
        ...
        <div class="popover-footer" :class="{ 'has-divider': dictList.length }">
          <div v-if="wordCollectPickerState.creating" class="create-row">
            ...
            />
            <BaseIcon class="action-btn" title="取消" @click="cancelCreating">
