...
import { ref } from 'vue'
import { getPracticeArticleCacheLocal, PRACTICE_ARTICLE_CACHE, PRACTICE_WORD_CACHE } from '../utils/cache'
import { usePracticeWordPersistence } from '../composables/usePracticePersistence.ts'
...
export function useExport() {
  ...
  async function getExportedData() {
    ...
    // 文章练习已裁剪,直接读本地缓存(保持导出结构兼容旧备份导入)
    let d1 = await getPracticeArticleCacheLocal()
    if (d1) {
